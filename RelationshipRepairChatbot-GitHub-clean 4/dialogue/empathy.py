...
LOW_GAIN_FALLBACK = (
    "这件事涉及具体行为、对方反应和后续沟通停顿。\n"
    "接下来核对对方反应和目前沟通状态，后续处理会更有依据。"
)

LOW_LOSS_FALLBACK = (
    "这件事涉及具体行为、对方反应和后续沟通停顿。\n"
    "若责任点、对方反应和沟通状态混在一起，问题可能被冲动处理并继续扩大。"
)
...
def infer_event_focus(user_statement, task_condition="victim"):
    """Create a short neutral event focus for low-empathy fallback replies."""
    text = user_statement or ""

    if any(term in text for term in ["没回家", "没有回家", "不见了", "找不到", "联系不上", "失联"]):
        return "这件事涉及联系中断、去向不明和现实安全确认。"
    if any(term in text for term in ["游戏", "打游戏", "小队", "队友"]):
        if any(term in text for term in ["施压", "教训", "骂", "指责", "逼", "压力"]):
            return "这件事涉及游戏中的表现分歧、持续施压和对方受伤反应。"
        return "这件事涉及游戏中的表现分歧、互动摩擦和后续疏远。"
    if any(term in text for term in ["拍照", "照片", "合照"]):
        return "这件事涉及相处细节、对方不满和后续沟通停顿。"
    if any(term in text for term in ["骗", "撒谎", "隐瞒", "没告诉", "没说", "喝酒", "出去"]):
        return "这件事涉及信息隐瞒、信任落差和后续沟通停顿。"
    if any(term in text for term in ["吼", "凶", "骂", "发火", "脾气", "生气"]):
        if task_condition == "transgressor":
            return "这件事涉及表达过重、对方受伤和后续沟通压力。"
        return "这件事涉及言语伤害、情绪冲突和后续沟通停顿。"
    if any(term in text for term in ["丢", "弄丢", "偷", "坏", "损坏", "相机"]):
        return "这件事涉及物品损失、责任承担和关系信任受损。"

    if task_condition == "transgressor":
        return "这件事涉及自身行为影响、对方反应和后续修复压力。"
    return "这件事涉及具体冲突、信任落差和后续沟通状态。"


def build_low_empathy_fallback(frame_condition, user_statement, task_condition="victim"):
    """Low-empathy support should be neutral but still relevant to the user input."""
    focus = infer_event_focus(user_statement, task_condition)
    compact = "".join((user_statement or "").split())
    has_detailed_context = len(compact) >= 35
    if frame_condition == "gain":
        if has_detailed_context:
            return f"{focus}\n接下来核对对方反应和目前沟通状态，后续处理会更有依据。"
        return f"{focus}\n补充对方反应和目前沟通状态后，后续处理会更有依据。"
    if has_detailed_context:
        return f"{focus}\n若责任点、对方反应和沟通状态混在一起，问题可能被冲动处理并继续扩大。"
    return f"{focus}\n若对方反应和沟通状态不清楚，问题可能被冲动处理并继续扩大。"
...
IMA要求：保留问题聚焦与情境评估功能：承接用户事件，指出事件与信任/沟通判断有关，说明核对缺口信息的价值。
...
如果用户已经描述了较完整经过，不得再说“先梳理发生经过”，只能说“核对对方反应和目前沟通状态”。
...
修辞-情绪状态：不得询问用户感受或情绪状态。
修辞-关怀陈述：不得包含关怀、理解、陪伴或安慰表达。
IMA要求：保留问题聚焦与情境评估功能：承接用户事件，指出事件与信任/沟通风险有关，说明缺口信息混在一起可能带来的负面后果。
输出格式：严格两行。第一行定位事件性质；第二行说明信息不清时可能造成的关系风险。
必须像聊天里的简短回应，不得像研究报告或总结表述。
必须贴合用户输入中的真实事件，不得写“冲突可能导致关系进一步恶化”“该问题需要被处理”“冲突起因是……”这类空泛或报告式句子。
如果用户已经描述了较完整经过，不得再说“若发生经过不清楚”，只能说“若责任点、对方反应和沟通状态混在一起……”。
只能做中性过渡，不给具体建议，不询问情绪。
不要出现“实验、阶段、任务、框架、研究”等后台词。
长度 45-85 个汉字。
""".strip(),
...
def get_emotional_support(empathy_condition, frame_condition, user_statement, task_condition="victim"):
    ...
    if empathy_condition == "low":
        fallback = build_low_empathy_fallback(frame_condition, user_statement, task_condition)
        return fallback
    text = generate_controlled_reply(
        stage_name,
        f"{instruction}\n用户当前关系角色：{task_condition}。低共情时仍需贴合该角色，不得把所有事件写成隐瞒、失约或信任落差。",
