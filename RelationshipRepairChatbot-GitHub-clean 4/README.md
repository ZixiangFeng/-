...
- 数据记录：整段对话保存到 `data/conversations.csv`，逐轮消息保存到 `data/conversation_turns.csv`

## 页面流程

```text
/             实验封面页
/randomizer   实验人员随机分组模拟器
/setup        设置关系角色、信息框架和共情条件
/chat         聊天机器人界面
/finish       结束页
```
...
- `data/conversations.csv`：每位被试一行，保存整段用户消息和 AI 消息，适合快速整理完整对话。
- `data/conversation_turns.csv`：每条消息一行，保存时间、实验条件、发送者、标签、是否有效输入和消息文本，适合正式分析。

正式部署后可以设置环境变量 `ADMIN_DOWNLOAD_KEY`，然后通过以下地址下载数据：

```text
/admin/download/conversations.csv?key=你的密钥
/admin/download/conversation_turns.csv?key=你的密钥
```

这些文件用于实验记录，不建议上传到 GitHub。
