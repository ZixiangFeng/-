...
TURN_CSV_PATH = os.path.join(DATA_DIR, "conversation_turns.csv")
...
def ensure_csv_exists():
    ...
    if not os.path.exists(CSV_PATH):
        with open(CSV_PATH, "w", newline="", encoding="utf-8") as csv_file:
            ...
            writer.writeheader()
    if not os.path.exists(TURN_CSV_PATH):
        with open(TURN_CSV_PATH, "w", newline="", encoding="utf-8") as csv_file:
            writer = csv.DictWriter(csv_file, fieldnames=TURN_CSV_FIELDS)
            writer.writeheader()
...
def log_turn(sender, text, labels=None, accepted=True):
    """Persist each user or chatbot message immediately for formal data retention."""
    if "participant_id" not in session:
        return

    ensure_csv_exists()
    turn_id = int(session.get("turn_id", 0)) + 1
    session["turn_id"] = turn_id
    row = {
        "participant_id": session["participant_id"],
        "turn_id": turn_id,
        "turn_timestamp": datetime.now(UTC).isoformat(timespec="seconds"),
        "task": session.get("task", ""),
        "frame": session.get("frame", ""),
        "empathy": session.get("empathy", ""),
    ...
    with open(TURN_CSV_PATH, "a", newline="", encoding="utf-8") as csv_file:
        writer = csv.DictWriter(csv_file, fieldnames=TURN_CSV_FIELDS)
        writer.writerow(row)
...
def append_user_message(message):
    ...
    log_turn("user", message, accepted=True)
...
def append_bot_turn(message):
    ...
    log_turn("bot", message["text"], labels=message.get("labels", []), accepted=True)
    return message
...
def next_bot_turn(user_message):
    ...
    if state == STATE_WAIT_INITIAL_PROBLEM:
        ...
        support = get_emotional_support(session["empathy"], session["frame"], user_message, session["task"])
        question = get_exploration_turn(0, task_condition=session["task"], initial_statement=user_message)
...
def setup():
    preset = {
        ...
        "empathy": request.args.get("empathy", ""),
    }
    if request.method == "POST":
        ...
        if task not in VALID_TASKS or frame not in VALID_FRAMES or empathy not in VALID_EMPATHY:
            return render_template(
                "setup.html",
                error="请选择关系角色、信息框架和共情沟通条件。",
                preset=preset,
            )
    ...
    if preset["task"] not in VALID_TASKS:
        preset["task"] = ""
    if preset["frame"] not in VALID_FRAMES:
        preset["frame"] = ""
    if preset["empathy"] not in VALID_EMPATHY:
        preset["empathy"] = ""
...
def api_message():
    ...
    if not is_valid:
        log_turn("user", message, accepted=False)
        log_turn("bot", retry_message, labels=["Clarification"], accepted=False)
        return jsonify(
            {
...
def api_llm_turn():
    ...
    if stage == "emotional_support":
        text = get_emotional_support(empathy, frame, user_message, role)
...
def admin_download(filename):
    admin_key = os.environ.get("ADMIN_DOWNLOAD_KEY", "")
    if not admin_key or request.args.get("key") != admin_key:
        return jsonify({"error": "未授权访问。请在环境变量 ADMIN_DOWNLOAD_KEY 中设置下载密钥。"}), 403

    files = {
        "conversations.csv": CSV_PATH,
        "conversation_turns.csv": TURN_CSV_PATH,
    }
    if filename not in files:
        return jsonify({"error": "文件不存在。"}), 404
    ensure_csv_exists()
    return send_file(files[filename], as_attachment=True, download_name=filename)
