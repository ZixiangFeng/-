...
from dialogue.scenarios import get_scenario, get_welcome_message
...
STATE_RETRY_MESSAGES = {
    ...
    STATE_WAIT_CAUSE: "我还不太清楚这次矛盾是怎么开始的。可以具体说说起因吗？",
...
def start_turn():
    ...
    return [append_bot_turn(make_bot_message(text, ["Opening"]))]
...
def next_bot_turn(user_message):
    ...
    if state == STATE_WAIT_INITIAL_PROBLEM:
        ...
        support = get_emotional_support(session["empathy"], user_message)
        question = get_exploration_turn(0)
    ...
    if state == STATE_WAIT_CAUSE:
        ...
        question = get_exploration_turn(1, user_message)
    ...
    if state == STATE_WAIT_HISTORY:
        ...
        exploration_answers.append(user_message)
        session["exploration_answers"] = exploration_answers
        question = get_exploration_turn(2, user_message)
...
def api_llm_turn():
    ...
    if stage == "emotional_support":
        text = get_emotional_support(empathy, user_message)
    elif stage == "exploration_turn_1":
        text = get_exploration_turn(1, user_message)
    elif stage == "exploration_turn_2":
        text = get_exploration_turn(2, user_message)
        ...
        text = get_advice_reinforcement(frame, user_message, context)
    else:
